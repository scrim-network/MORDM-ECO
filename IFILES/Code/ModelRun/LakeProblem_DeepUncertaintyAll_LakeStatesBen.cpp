/* LakeProblem_DeepUncertaintyAll_LakeStatesBen.cpp
   
   Riddhi Singh, Feb, 2014
   The Pennsylvania State University
   rus197@psu.edu

   Estimating the time series of phosphorus in the lake using the solution.
   The solution is from the 1 objective optimization under stochastic uncertainty.
   The solution is labelled 'MEU'- maximization of expected utiltiy
  ...
  Solution Number
  Not needed - there is only 1 solution for a single objective optimization
 ...
 Input Files
      ...
      File containing the decision vectors from the optmization done before, there are 10, one for each parallel seed. 
      The first decision vector from this file is used to generate output files 

 Outputs
  
  Output Files
   Objectives estimated for different assumptions of uncertiainty:
   1. LakeProblem_1ObjBenStochMM11Apr_Type4_lakestate_Type0.txt to LakeProblem_1ObjBenStochMM11Apr_Type4_lakestate_Type8.txt
...
int main(int argc, char* argv[]) 
{
  ...
  string path           = "/Data/";
  string solheader      = "LakeProblem_1ObjStochMM11April_Type4";
  string SOWheader      = "SOWs_Type";
  string solext         = "_Par_Pareto";
  string ext            = ".txt";
  string outheader      = "LakeProblem_1ObjStochMM11April_Type4";
 ...
 for (int solfnum=0; solfnum<nfiles; solfnum++)
   {
	...
	{ linenumSOW++;
     ...
     int solnum = 1;    //only 1 solution since its a single objective and we are interested in the first random seed 
     //now open the output file and the file whose solutions are to be evaluated for these states of the world 
     outfilename = path + outheader + outext + tempind + ext;
