/* LakeProblem_DeepUncertainty_Reevaluate_BenAll.cpp
   
   Riddhi Singh, Feb, 2014
   The Pennsylvania State University
   rus197@psu.edu

   Reevlauating the solutions obtained by optimizing the single objective Bentham utiltiy with/without stochastic uncertainty.
   This reevaluation is against all the 9 lognormal distributions, each represented by 10000 states of the world (SOWs). 
   All the random  SOWs are in the same file, resulting in 90000 SOWs.
  ...
  Input Files
      1. SOWs_TypeAll.txt : a file containing all the SOWs against which the solutions are to be evaluated.
      2. LakeProblem_1ObjStochMM11April_Type4_Par_Pareto.txt/LakeProblem_1ObjDetMM11April_Type4_Par_Pareto.txt: 
      Files containing the decision vectors from the optmization done before. 
      The first file is for the solution of single objective optmization with stochastic uncertainty.
      The second file is for the same but without stochastic uncertainty. 
  ...
  Outputs
   ...
   Output Files
   1. LakeProblem_1ObjStochMM11April_Type4_Reev_TypeAll.txt/LakeProblem_1ObjDetMM11April_Type4_Reev_TypeAll.txt -
   Contains the reevaluated objectives for each case. 
   Note that since we had 10 random seeds per optimization, there are 10 reevaluations. 
...
int main(int argc, char* argv[]) 
{
 ...
 string solfilename;
  ...
  for (int solfnum=0; solfnum<1; solfnum++)
    {
     ...
     outfilename = path + myheader + outext + "Type" + "All" + ext;
     solfilename = path + myheader + myext + ext;
