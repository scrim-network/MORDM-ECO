/* LakeProblem_1Obj_1Const_Det.cpp
   
   Riddhi Singh, Jan, 2014
   The Pennsylvania State University
   rus197@psu.edu

   Running BORG parallel for the deterministic lake model from Carpenter et al., 1999
...
void lake_problem(double* vars, double* objs, double* consts) 
{
  ...
  for (i=0; i<nDays; i++)
    {
      ofs1[0] = ofs1[0] + lake_stateX[i];
      ofs2[0] = ofs2[0] + utility[i]*pow(delta,(i));
      if (i==0)
	ofs3[0] = utility[i];
      if (i>49)
	ofs4[0] = ofs4[0]+utility[i];
  ...
  ofs1[0] = ofs1[0]/nDays;
  ofs4[0] = ofs4[0]/(nDays-50);
  objs[0] = ofs2[0];
  
  objs[0]    = -objs[0];     //want to maximize Benthams's utility
...
int main(int argc, char* argv[]) 
{
  ...
  BORG_Algorithm_ms_max_time(1);
  BORG_Algorithm_output_frequency(1000000);
  ...
  BORG_Problem_set_epsilon(problem, 0, 0.01);   //Benthams's utility
  ...
  for (i=0; i<10; i++) {
    ...
    sprintf(runtime, "runtime1ObjBenDetMM_Type4_%d_%%d.txt", i);
